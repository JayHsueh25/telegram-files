...
import telegram.files.telegram.TelegramChatFileService;
import telegram.files.telegram.TelegramDownloadService;
...
public class TelegramVerticle extends AbstractVerticle {
    ...
    private final TelegramChatFileService chatFileService = new TelegramChatFileService();

    private final TelegramDownloadService downloadService = new TelegramDownloadService();
    ...
    public Future<JsonObject> getChatFilesCount(long chatId) {
        return chatFileService.getChatFilesCount(client, chatId);
    }
    ...
    public Future<FileRecord> startDownload(Long chatId, Long messageId, Integer fileId) {
        return downloadService.startDownload(client,
                telegramRecord,
                chatId,
                messageId,
                fileId,
                this::syncFileDownloadStatus,
                this::sendEvent,
                this::getRootId);
    }

    public Future<Boolean> downloadThumbnail(Long chatId, Long messageId, FileRecord thumbnailRecord) {
        return downloadService.downloadThumbnail(client, chatId, messageId, thumbnailRecord, this::getRootId);
    }

    public Future<Void> cancelDownload(Integer fileId) {
        return downloadService.cancelDownload(client, fileId, this::sendEvent);
    }

    public Future<Void> togglePauseDownload(Integer fileId, boolean isPaused) {
        return downloadService.togglePauseDownload(client, fileId, isPaused, this::syncFileDownloadStatus);
    }

    public Future<Void> removeFile(Integer fileId, String uniqueId) {
        return downloadService.removeFile(client, fileId, uniqueId, this::sendEvent, this::getRootId);
    }

    public Future<Void> updateAutoSettings(Long chatId, JsonObject params) {
        return downloadService.updateAutoSettings(vertx, telegramRecord, chatId, params);
    }
