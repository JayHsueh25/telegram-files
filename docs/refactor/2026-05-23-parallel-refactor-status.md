...
- API full test: follow-up branch `codex/test-stability-after-refactor` now passes `cd api && .\gradlew.bat test`. Latest run exits with `BUILD SUCCESSFUL`; test result XML reports 97 tests, 0 failures, 0 errors, and 1 skipped.
...
- Telegram service extraction: completed for chat file query and download control services. Follow-up service-level tests now cover `TelegramChatFileService.getChatFilesCount` aggregation and `TelegramDownloadService.downloadThumbnail` missing/completed/existing-record branches using a mockable `TelegramClientGateway`.
- Test stability follow-up: completed. Gradle test defaults now provide safe fake Telegram credentials, a per-run `APP_ROOT`, and a test `DATA_PATH`; SQLite cleanup undeploys Vert.x deployments and deletes only the test DB cluster with retry; `MessyUtilsTest` uses manually managed unique fixture directories to avoid JUnit temp cleanup failures on Windows memory-mapped files; `FileDownloadStatusConcurrentTest` starts child JVMs with `--enable-preview` and asserts child process exit codes.
- Known risks:
  - API full-suite is green in the local SQLite test environment, but TDLib live integration still needs manual smoke testing with a real Telegram account before release.
